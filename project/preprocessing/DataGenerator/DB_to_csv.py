...
if __name__ == "__main__":
    ...
    imagefile = list(map(lambda x:x[-2], row))
    link = list(map(lambda x:x[-1], row))

    item_table = pd.DataFrame({'itemid': itemid, 'category' : category, 'imagefile' : imagefile, 'link' : link})
